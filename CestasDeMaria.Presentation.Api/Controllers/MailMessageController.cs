...
using IMainAppService = CestasDeMaria.Application.Interfaces.IMailMessageAppService;
...
namespace CestasDeMaria.Presentation.Api.Controllers
{
    ...
    [Authorize]
    [Route("MailMessage")]
    public class MailMessageController : ControllerBase, IDisposable
    {
        ...
        /// Class constructor
        /// </summary>
        public MailMessageController(IMainAppService mainAppService, IOptions<Settings> options, IHttpContextAccessor httpContextAccessor)
        {
        ...
        /// <returns><![CDATA[Task<MainViewModel>]]></returns>
        [HttpGet("resend/{code}")]
        public async Task<IActionResult> Resend(long code)
        {
            var mainDto = await _mainAppService.ResendMail(code);
