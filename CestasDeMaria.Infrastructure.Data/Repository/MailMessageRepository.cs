...
{
    public class MailMessageRepository : RepositoryBase<Main>, IMainRepository
    {
        public MailMessageRepository(CestasDeMariaContext currentContext)
            : base(currentContext)
        {
        }
        ...
        public async Task<Tuple<int, IEnumerable<Main>>> GetAllPagedAsync(int page, int quantity, string isActive = null, string term = null, string orderBy = null, string[] include = null)
        {
            ...
            {
                query = query.Where(c => c.To.Contains(term));
            }
